...
def _scheda_cliente(c, utente):
    ...
    with col_d:
        if st.button("Documenti", key=f"doc_{c['id']}"):
            ...
            st.rerun()
    with col_e:
        if st.button("Veicoli", key=f"vei_{c['id']}"):
            st.session_state.pagina = "veicoli"
            st.session_state.cliente_id = c["id"]
            st.session_state.cliente_nome = nome_display(c)
            st.rerun()
...
def _form_nuovo_cliente(utente):
    ...
    from bozze import salva_bozza, riquadro_bozze
    # riquadro bozze salvate (si riprendono a scelta; il form resta pulito)
    _ripresa = riquadro_bozze("cliente", utente, key_prefix="cli")
    _pre = _ripresa or {}
    with st.form("form_nuovo_cliente"):
        dati = _form_campi(tipo=tipo, d=_pre, key_prefix=("new_r" if _ripresa else "new"))
        col_crea, col_bz = st.columns([2, 1])
        submitted = col_crea.form_submit_button("Crea cliente", use_container_width=True)
        salva_bz = col_bz.form_submit_button("💾 Salva bozza", use_container_width=True)
    if salva_bz:
        import uuid as _uuid
        _ch = st.session_state.get("bozza_cliente_chiave") or str(_uuid.uuid4())
        st.session_state["bozza_cliente_chiave"] = _ch
        salva_bozza("cliente", _ch, dati, utente["id"],
                    titolo=f"Cliente: {dati.get('ragione_sociale') or dati.get('nome') or 'nuovo'}")
        st.success("💾 Bozza salvata nel cassetto qui sopra.")
    ...
    if submitted:
        ...
        if errori:
            ...
            # anti doppio-salvataggio: firma della scheda appena inviata
            import hashlib
            firma = hashlib.md5(
                (str(dati.get("nome",""))+str(dati.get("cognome",""))
                 +str(dati.get("ragione_sociale",""))+str(dati.get("telefono",""))
                 +str(dati.get("email",""))).encode()
            ).hexdigest()
            if st.session_state.get("ultimo_cliente_creato") == firma:
                st.warning("Questa scheda è già stata salvata un attimo fa.")
                st.stop()
            risultato = crea_cliente(dati, utente["id"])
            if risultato:
                st.session_state["ultimo_cliente_creato"] = firma
                from bozze import elimina_bozza
                from bozze import elimina_bozza
                # cancello la bozza usata: quella ripresa o quella salvata a mano
                ch = (st.session_state.get("bozza_cliente_ripresa")
                      or st.session_state.get("bozza_cliente_chiave"))
                if ch:
                    try:
                        elimina_bozza("cliente", ch)
                    except Exception:
                        pass
                st.session_state.pop("bozza_cliente_chiave", None)
                st.session_state.pop("bozza_cliente_ripresa", None)
                st.session_state.pop("ripresa_cliente_cli", None)
