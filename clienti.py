...
def _form_nuovo_cliente(utente):
    ...
    _bz = carica_bozza("cliente", "nuovo")
    _pre = (_bz.get("contenuto") if _bz else {}) or {}
    if _bz and not st.session_state.get("bz_cliente_nuovo"):
        a = _bz.get("autore") or {}
        chi = f"{a.get('nome','')} {a.get('cognome','')}".strip()
        st.info(f"📄 Ripresa bozza scheda cliente{' di ' + chi if chi else ''}.")
        st.session_state["bz_cliente_nuovo"] = True
    with st.form("form_nuovo_cliente"):
        dati = _form_campi(tipo=tipo, d=_pre, key_prefix="new")
    ...
    if salva_bz:
        salva_bozza("cliente", "nuovo", dati, utente["id"],
