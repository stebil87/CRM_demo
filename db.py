...
def fmt_data(valore, formato="%d/%m/%Y %H:%M"):
    """Converte un timestamp del database (UTC, ISO) in ora svizzera e lo
    formatta. Robusto verso stringhe con 'Z', microsecondi o senza fuso.
    Ritorna '' se il valore non e' interpretabile."""
    if not valore:
        return ""
    try:
        if isinstance(valore, datetime):
            ...
            s = str(valore).replace("Z", "+00:00")
            dt = datetime.fromisoformat(s)
        # se non ha fuso, il database salva in UTC: lo dichiaro tale
        ...
        return str(valore)[:16].replace("T", " ")
...
@st.cache_data(ttl=60)
@profila
def followup_oggi(utente_id=None):
    sb = get_sb()
    try:
        oggi = date.today().isoformat()
        q = sb.table("diario").select(
            "*, clienti(nome, cognome, ragione_sociale, tipo)"
        ).eq("followup_fatto", False).eq("followup_data", oggi)
        if utente_id:
            q = q.or_(f"followup_assegnato_a.eq.{utente_id},followup_assegnato_a.is.null")
        res = q.execute()
        return res.data or []
    except:
        return []
