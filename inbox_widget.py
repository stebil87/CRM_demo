...
def _e_richiesta_sito(e):
    ogg = str(e.get("oggetto", ""))
    flag_sito = ("[Sito]", "[Carrozzeria]", "[Compravendita]", "[Officina]", "[Gomme]", "[Valutazione]")
    return ogg.startswith(flag_sito)
...
def _widget_lista(utente, nuove, titolo, vuoto, colore, chiave):
    ...
    intestazione(titolo, "🌐" if chiave == "sito" else "📧",
                 badge=(f"{len(nuove)} nuove" if nuove else None),
                 badge_bg=colore, destra="Tutto gestito")
...
def controlla_posta():
    ...
    try:
        ...
        for mid in ids[:30]:  # massimo 30 per volta, per non bloccare la pagina
            ...
            if oggetto.strip().startswith(("[Sito]","[Carrozzeria]","[Compravendita]","[Officina]","[Gomme]","[Valutazione]")):
...
def pagina_inbox(utente):
    ...
    tab_nuove, tab_storico, tab_scrivi = st.tabs([
        "Da gestire", "Storico", "✉️ Scrivi email"
    ])
    ...
    with tab_storico:
        ...
        if not storico:
            ...
            for e in storico:
                _email_storico(e, utente)

    with tab_scrivi:
        from email_composer import compositore_email
        compositore_email(key="email_nuova", titolo="Scrivi una nuova email", utente=utente)
