...
def _widget_lista(utente, nuove, titolo, vuoto, colore, chiave):
    ...
    for e in nuove:
        ...
        if st.session_state.get(f"inbox_open_{chiave}_{e['id']}"):
            ...
            # risposta rapida direttamente qui (dashboard o pagina)
            from email_composer import compositore_email
            if st.session_state.get(f"risposto_{chiave}_{e['id']}"):
                st.success("✅ Risposta inviata e presa in carico.")
            else:
                def _dopo(_e=e, _ch=chiave):
                    if not _e.get("presa_in_carico"):
                        prendi_in_carico_email(_e["id"], utente["id"])
                    st.session_state[f"risposto_{_ch}_{_e['id']}"] = True
                    st.session_state[f"inbox_open_{_ch}_{_e['id']}"] = False
                    st.rerun()
                compositore_email(
                    key=f"dash_{chiave}_{e['id']}",
