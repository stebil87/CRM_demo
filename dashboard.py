...
def pagina_dashboard(utente):
    ...
    # ── KPI OPERATIVI (garage) ──
    from db import get_sb
    sb = get_sb()
    def _conta(tab, filtro=None):
        try:
            q = sb.table(tab).select("id", count="exact")
            if filtro:
                q = filtro(q)
            return q.execute().count or 0
        except Exception:
            return 0
    n_clienti = _conta("clienti")
    n_veicoli = _conta("veicoli")
    n_gomme = _conta("deposito_gomme", lambda q: q.eq("stato", "in deposito"))
    n_fu_oggi = len(oggi_fu)

    cards = [
        ("Clienti", n_clienti, "#1a1a2e", "in anagrafica"),
        ("Veicoli", n_veicoli, "#0f3460", "registrati"),
        ("Gomme in deposito", n_gomme, "#F97316", "set a magazzino"),
        ("Follow-up oggi", n_fu_oggi, "#e94560", "da gestire"),
    ]
    cols = st.columns(4)
    for col, (tit, val, colore, sub) in zip(cols, cards):
        with col:
            st.markdown(
                "<div style='background:white;border:1px solid #eaeaf0;"
                f"border-top:4px solid {colore};border-radius:10px;"
                "padding:20px 24px;box-shadow:0 1px 4px rgba(0,0,0,0.06);'>"
                "<div style='font-size:11px;font-weight:700;text-transform:uppercase;"
                f"letter-spacing:0.8px;color:#888;margin-bottom:8px;'>{tit}</div>"
                f"<div style='font-size:32px;font-weight:700;color:#1a1a2e;'>{val}</div>"
    ...
    with col_inbox:
        ...
        with st.popover("✉️ Scrivi email", use_container_width=True):
            if compositore_email(key="dash_nuova", titolo="", compatto=True):
                pass
    ...
    st.markdown("---")
    st.markdown("**Clienti per stato**")
    if not clienti_df.empty and "stato" in clienti_df.columns:
        import plotly.express as px
        stato_count = clienti_df["stato"].value_counts().reset_index()
        stato_count.columns = ["Stato", "Numero"]
        fig = px.pie(
            stato_count, names="Stato", values="Numero", hole=0.5,
