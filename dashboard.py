...
from ui_stile import stile_globale, intestazione
...
def _widget_messaggi(utente):
    ...
    intestazione("Messaggi interni", "💬",
                 badge=(f"{len(non_letti)} non letti" if non_letti else None),
                 destra="Tutti letti")
...
def pagina_dashboard(utente):
    ...
    with col_fu_oggi:
        intestazione("Follow-up di oggi", "✅")
        if oggi_fu:
            for f in oggi_fu:
    ...
    with col_fu_prox:
        intestazione("Follow-up prossimi 7 giorni", "🗓️")
        if prossimi_fu:
    ...
    intestazione("Compleanni in arrivo", "🎂",
                 badge=(f"{len(compleanni)} nei prossimi 30 giorni" if compleanni else None),
                 badge_bg="#856404")
    if not compleanni:
        st.markdown(
    ...
    from appuntamenti import widget_richieste_appuntamento
    col_sito, col_mail, col_app = st.columns(3)
    with col_sito:
        widget_richieste_sito(utente)
    with col_mail:
        widget_inbox(utente)
    with col_app:
        widget_richieste_appuntamento(utente)
    ...
    with col_az1:
        intestazione("Scrivi una nuova email", "✉️")
        with st.popover("Apri compositore", use_container_width=True):
            compositore_email(key="dash_nuova", titolo="", compatto=True, utente=utente)
    with col_az2:
        intestazione("Nuovo follow-up", "🔔")
    ...
    from bozze import widget_bozze_in_sospeso
    _bz_pending = widget_bozze_in_sospeso(utente)
