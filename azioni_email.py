...
def form_nuovo_followup(utente, key="dash"):
    ...
    with st.form(f"funew_{key}"):
        cli = st.selectbox("Cliente", opz_cli, format_func=lambda x: x[1],
                           help="Scegli un cliente registrato o «non registrato» e scrivi il nome sotto.")
        nome_libero = st.text_input(
            "Nome cliente (se non registrato)",
            placeholder="Compila solo se hai scelto «Cliente non registrato»")
        titolo = st.text_input("Oggetto *", placeholder="Es. richiamare per preventivo")
        col1, col2, col3 = st.columns(3)
        with col1:
            quando = st.date_input("Data promemoria *", value=date.today())
        with col2:
            ora = st.time_input("Ora", value=None)
        with col3:
    ...
    if ok:
        ...
        if not cli[0] and nome_libero.strip():
            contenuto = (contenuto + "\n\n[Cliente non registrato: "
                         + nome_libero.strip() + "]").strip()
        if ora:
            contenuto = (f"Ore {ora.strftime('%H:%M')}. " + contenuto).strip()
        dati = {
