...
def _tab_appuntamento(e, utente, key, cliente_id):
    ...
    with st.form(f"app_{key}"):
        titolo = st.text_input("Titolo *", value=f"Appuntamento — {e.get('mittente','')[:40]}")
...
def _tab_followup(e, utente, key, cliente_id):
    if st.session_state.get(f"fu_ok_{key}"):
        ...
        return
    if not cliente_id:
        st.info("Questa email non è collegata a una scheda cliente, "
                "quindi il follow-up non può essere agganciato. "
                "Rispondi pure: la scheda verrà creata al primo contatto salvato.")
        return
    opz = _opzioni_utenti_calendario(utente)
