...
from db import lista_diario, crea_voce_diario, aggiorna_voce_diario, lista_documenti, scarica_documento, fmt_data
...
def _sezione_note(v, utente):
    ...
    for n in note:
        ...
        data = fmt_data(n.get("created_at"), "%d/%m/%Y %H:%M")
        st.markdown(
...
def _form_nuova_voce(utente, cliente_id):
    from db import lista_utenti, get_calendari_modificabili
    from bozze import carica_bozza, salva_bozza, elimina_bozza
    st.subheader("Nuova voce")
    _bkey = f"diario_{cliente_id}"
    _bz = carica_bozza("nota", _bkey)
    _pre = (_bz.get("contenuto") if _bz else {}) or {}
    if _bz and not st.session_state.get(f"bz_nota_{cliente_id}"):
        st.info("📄 Bozza ripresa.")
        st.session_state[f"bz_nota_{cliente_id}"] = True
    ids_mod = get_calendari_modificabili(utente["id"])
    opz_utenti = [(utente["id"], "io")]
    for u in (lista_utenti() or []):
        if u["id"] in ids_mod and u["id"] != utente["id"]:
            opz_utenti.append((u["id"], f"{u.get('nome','')} {u.get('cognome','')}".strip()))
    with st.form("form_nuova_voce"):
        col1, col2 = st.columns(2)
        with col1:
            tipo = st.selectbox("Tipo", TIPI)
            titolo = st.text_input("Titolo *", value=_pre.get("titolo",""))
        with col2:
        ...
        contenuto = st.text_area("Dettaglio / Nota", height=150,
            value=_pre.get("contenuto",""),
            placeholder="Testo libero: comparirà nel follow-up in dashboard.")
        assegnato = st.selectbox("Follow-up assegnato a", opz_utenti,
            format_func=lambda x: x[1],
            help="Chi vedrà il promemoria nella sua dashboard.")
        col_add, col_bz = st.columns([2, 1])
        submitted = col_add.form_submit_button("Aggiungi", use_container_width=True)
        salva_bz = col_bz.form_submit_button("💾 Salva bozza", use_container_width=True)
    if salva_bz:
        salva_bozza("nota", _bkey, {"titolo": titolo, "contenuto": contenuto},
                    utente["id"], titolo=f"Nota: {titolo or 'senza titolo'}")
    ...
    if submitted:
        if not titolo:
            ...
            crea_voce_diario({
                ...
                "followup_assegnato_a": assegnato[0] if followup_data else None,
            }, utente["id"])
            try:
                elimina_bozza("nota", _bkey)
            except Exception:
                pass
